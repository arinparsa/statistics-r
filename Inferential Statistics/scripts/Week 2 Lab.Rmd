...
samp %>% summarise(samp_med = median(area))
...
```{r ci}
samp %>%
  summarise(lower = (mean(price)) - (z_star_95 * (sd(price) / sqrt(n))),
            upper = (mean(price)) + (z_star_95 * (sd(price) / sqrt(n))))
```
...
<li> 5% confident that the sample mean is in this interval. </li>
</ol>

*Confidence intervals mean that 95% of random sample with a certain size will have confidence intervals that contain the true average of what is being asked.   
