...
<li> 50\% of houses in Ames are smaller than 1,499.69 square feet. </li> - False
...
samp1 %>% summarise(samp_mean = mean(area), samp_med = median(area), 
...
```{r sampling-dist}
# type your code for the Exercise here, and Run Document
sample_means50 %>% summarise(samp_mean = mean(x_bar))
```
...
samp_price <- ames %>% sample_n(size = 50) %>%
...
```{r price-sampling}
# type your code for this Exercise here, and Run Document
sample_means50_2 <- ames %>%
                    rep_sample_n(size = 50, reps = 5000, replace = TRUE) %>%
                    summarise(x_bar = mean(price))

ggplot(data = sample_means50_2, aes(x = x_bar)) +
...
# type your code for this Exercise here, and Run Document
samp_price <- ames %>% sample_n(size = 10) %>%
  summarise(x_bar = mean(price))
...
price <- ames %>% select(price) %>% summarise(pop_mean=mean(price))
